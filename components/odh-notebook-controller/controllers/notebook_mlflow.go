...
func mlflowRoleBindingName(notebook *nbv1.Notebook) string {
	return fmt.Sprintf("%s-%s-%s", notebook.Name, notebook.Namespace, MLflowIdentifier)
}
...
// NewMLflowRoleBinding defines the desired RoleBinding object for MLflow access.
// The RoleBinding references the MLflow ClusterRole but is namespace-scoped for automatic cleanup.
func NewMLflowRoleBinding(notebook *nbv1.Notebook) *rbacv1.RoleBinding {
	return &rbacv1.RoleBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name:      mlflowRoleBindingName(notebook),
...
func (r *OpenshiftNotebookReconciler) ReconcileMLflowRoleBinding(notebook *nbv1.Notebook, ctx context.Context) error {
	...
	desiredRoleBinding := NewMLflowRoleBinding(notebook)
	...
	// Update RoleBinding if Subjects, Labels, or OwnerReferences differ. Copy ResourceVersion and UID
	// from found to desired so Update succeeds.
	...
	if needsUpdate {
		log.Info("Updating MLflow RoleBinding", "RoleBinding.Namespace", desiredRoleBinding.Namespace, "RoleBinding.Name", desiredRoleBinding.Name)
		desiredRoleBinding.ResourceVersion = foundRoleBinding.ResourceVersion
		desiredRoleBinding.UID = foundRoleBinding.UID
		err = r.Update(ctx, desiredRoleBinding)
		if err != nil {
			log.Error(err, "Failed to update MLflow RoleBinding", "RoleBinding.Namespace", desiredRoleBinding.Namespace, "RoleBinding.Name", desiredRoleBinding.Name)
			return err
		}
	}
...
func HandleMLflowEnvVars(ctx context.Context, cli client.Client, dynamicClient dynamic.Interface, notebook *nbv1.Notebook, log logr.Logger) error {
	...
	err = setNotebookContainerEnvVar(notebook, MLflowTrackingURIEnvVar, trackingURI)
	if err != nil {
