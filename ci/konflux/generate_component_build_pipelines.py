...
"""
We have a couple of components and their pipeline specs are very repetitive.

This script creates the Tekton pipelines under /.tekton

Usage:

$ poetry run ci/konflux/generate_component_build_pipelines.py
"""
...
def component_build_pipeline(component_name, context: str, dockerfile_path: str,
    ...
    return {
        ...
        "spec": {
            "params": [
                ...
                *([{"name": "path-context", "value": context}] if context else []),
                *([{"name": "dockerfile", "value": dockerfile_path}] if dockerfile_path else []),
            ],
            "pipelineSpec": {
...
def main():
    context = "components"
    for task_name, dockerfile in [
        ("kf-notebook-controller", "notebook-controller/Dockerfile"),
        ("odh-notebook-controller", "odh-notebook-controller/Dockerfile"),
    ]:
        with open(ROOT_DIR / ".tekton" / (task_name + "-odh-io-main" + "-on-push.yaml"), "w") as yaml_file:
            print("# yamllint disable-file", file=yaml_file)
            print("# This file is autogenerated by ci/konflux/generate_component_build_pipelines.py", file=yaml_file)
            print(yaml.dump(component_build_pipeline(component_name=task_name,
                                                     context=context, dockerfile_path=dockerfile,
                                                     build_container_tasks=[build_container()], is_pr=False)),
                  file=yaml_file)
        with open(ROOT_DIR / ".tekton" / (task_name + "-odh-io-main" + "-pull-request.yaml"), "w") as yaml_file:
            print("# yamllint disable-file", file=yaml_file)
            print("# This file is autogenerated by ci/konflux/generate_component_build_pipelines.py", file=yaml_file)
            print(yaml.dump(component_build_pipeline(component_name=task_name,
                                                     context=context, dockerfile_path=dockerfile,
